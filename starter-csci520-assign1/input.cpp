...
void getMouseWorldCoordinates(int mouseX, int mouseY, double &worldX, double &worldY, double &worldZ)
{
    // get viewport info
    GLint viewport[4]; // (0, 1): bottom-left corner x,y; (2, 3): width, height
    ...
    // convert GLUT viewport coord to OpenGL screen coord
    double winX = double(mouseX);
    double winY = double(viewport[3] - 1 - mouseY);

    // calculate depth
    float depth;
    glReadPixels(mouseX, viewport[3] - mouseY - 1, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &depth);

    // calculate world coord
    gluUnProject(winX, winY, depth, modelview, projection, viewport, &worldX, &worldY, &worldZ);
...
void mouseMotionDrag(int x, int y)
{
  ...
  // implement left button mouse drag
  if (g_iLeftMouseButton)
  {
      // OutputDebugStringA("Left Button Drag\n");

      double mouseEndPosX, mouseEndPosY, mouseEndPosZ;
      getMouseWorldCoordinates(x, y, mouseEndPosX, mouseEndPosY, mouseEndPosZ);

      /*double dragX = (x - g_mouseStartPos[0]);
      double dragY = -(y - g_mouseStartPos[1]);*/

      // drag force in world space
      dragForce[0] = mouseEndPosX - mouseStartPosW[0];
      dragForce[1] = mouseEndPosY - mouseStartPosW[1];
      dragForce[2] = mouseEndPosZ - mouseStartPosW[2];

      // update start world pos
      mouseStartPosW[0] = mouseEndPosX;
      mouseStartPosW[1] = mouseEndPosY;
      mouseStartPosW[2] = mouseEndPosZ;
...
void mouseButton(int button, int state, int x, int y)
{
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
      ...
      if (state == GLUT_DOWN)
      {
          g_iLeftMouseButton = 1; // clicked

          /*g_mouseStartPos[0] = x;
          g_mouseStartPos[1] = y;*/
          getMouseWorldCoordinates(x, y, mouseStartPosW[0], mouseStartPosW[1], mouseStartPosW[2]);
