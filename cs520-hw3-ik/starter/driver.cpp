...
static const double recordingFPS = 15.0;
static double recordingInterval = 1.0 / recordingFPS;
static double recordingTimer = 0.0;
...
static void saveScreenshotPPM(int frameID)
{
    int w = windowWidth;
    int h = windowHeight;

    // read pixels of current frame
    vector<unsigned char> pixels(3 * w * h);
    glReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, pixels.data());

    char filename[256];
    sprintf(filename, "../screenshots/frame_%04d.ppm", frameID);
...
static void idleFunction()
{
  ...
  counter.StopCounter();
  double dt = counter.GetElapsedTime(); // time each frame
