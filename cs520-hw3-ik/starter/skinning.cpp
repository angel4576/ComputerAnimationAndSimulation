...
{
    // DeformedPos = Sigma_j (weight_j * M_j * restPos) 
    for (int i = 0; i < numMeshVertices; i++)
    {
        Vec3d restPos(
        ...
        Vec3d deformPos(0.0, 0.0, 0.0);

        for (int j = 0; j < numJointsInfluencingEachVertex; j++) // bones 
        {
            // Joints that affect vertex i
            int jointIdx = meshSkinningJoints[i * numJointsInfluencingEachVertex + j];
            double weight = meshSkinningWeights[i * numJointsInfluencingEachVertex + j];

            if (weight == 0.0) // no contribution
            {
                continue;
            }

            const RigidTransform4d& skinTransform = jointSkinTransforms[jointIdx];
            Vec3d newPos = skinTransform.transformPoint(restPos);
            
            deformPos += weight * newPos;
        }

        newMeshVertexPositions[3 * i + 0] =  deformPos[0];
        newMeshVertexPositions[3 * i + 1] =  deformPos[1];
        newMeshVertexPositions[3 * i + 2] =  deformPos[2];
